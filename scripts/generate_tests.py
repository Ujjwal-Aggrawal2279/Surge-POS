...
def generate_test_file(api_stem: str, file_data: dict) -> str:
    ...
    if all_paths:
        ...
        lines.append("])")
        lines.append("def test_validation_path_has_integration_coverage(path_id, endpoint, error_fragment, request):")
        lines.append("    '''Each frappe.throw() path must have an integration test that matches the message.'''")
    ...
    unguarded = [
        fn for fn, data in endpoints.items()
        if not data["throws"] and not data["guards"]
    ]
    if unguarded:
