...
A03  Wrong PIN 3× sequential — locked, lockout_until shown
A04  Wrong PIN 3× concurrent — atomic INCR, exactly 3 increments
...
def _add_to_profile(user, access_level="Cashier", pin=_TEST_PIN_HASH, status="Active"):
    existing = frappe.db.exists(
        "POS Profile User", {"parent": _TEST_PROFILE, "user": user}
    )
    if existing:
        frappe.db.set_value(
            "POS Profile User",
...
class TestPINLogin(AuthTestBase):
    ...
    def test_A03_wrong_pin_3x_sequential_locks_account(self):
        """A03: 3× wrong PIN sequential → locked, lockout_until shown."""
    ...
    def test_A04_wrong_pin_3x_concurrent_atomic(self):
        ...
        self.assertTrue(_is_locked(_CASHIER, _TEST_PROFILE), "User must be locked after 3 concurrent wrong PINs")
    ...
    def test_A06_supervisor_unlocks_cashier(self):
        ...
        result = frappe.parse_json(
            override_lockout(_CASHIER, _TEST_PIN_HASH, _TEST_PROFILE).data
        )
    ...
    def test_A11_plaintext_pin_migration(self):
        ...
        self.assertTrue(_is_hashed(stored), "PIN must be auto-upgraded to hash on first successful migration login")
    ...
    def test_A14_disabled_user_returns_invalid(self):
        ...
        result = frappe.parse_json(
            verify_pin(_TEST_PROFILE, _DISABLED_USER, _TEST_PIN_HASH).data
        )
