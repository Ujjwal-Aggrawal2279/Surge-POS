...
F01  net_sales_paise = sum(grand_total for non-returns) x 100
F02  net_returns_paise = sum(grand_total for is_return) x 100
...
def _make_entry(balance_details=None, period_start=None):
	return SimpleNamespace(
		...
		balance_details=[
			SimpleNamespace(mode_of_payment=k, opening_amount=v) for k, v in (balance_details or {}).items()
		],
	)
...
class TestZReportMath(ZReportBase):
	...
	def test_F03_per_mode_amounts_summed(self):
		...
		report = _build_z_report(
			_make_entry(balance_details={"Cash": 0.0}), [{"mode_of_payment": "Cash", "amount": 60.0}], ""
		)
