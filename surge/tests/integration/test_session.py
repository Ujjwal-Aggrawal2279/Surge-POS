...
from surge.api.session import open_session, close_session, get_active_session
...
class TestShiftOpen(FrappeTestCase):
    ...
    def test_B03_stale_session_previous_day(self):
        ...
        frappe.db.set_value(
            "POS Opening Entry", opened["session_name"], "period_start_date", yesterday
        )
        frappe.db.commit()
...
class TestShiftClose(FrappeTestCase):
    ...
    def test_E11_paise_rounding_10_10(self):
        ...
        self.assertEqual(int(round(10.10 * 100)), 1010)
    ...
    def test_E14_system_manager_can_close_session(self):
        ...
        try:
            result = frappe.parse_json(
                close_session(
                    opened["session_name"], [{"mode_of_payment": "Cash", "amount": 0}]
                ).data
            )
        finally:
            frappe.set_user("Administrator")
