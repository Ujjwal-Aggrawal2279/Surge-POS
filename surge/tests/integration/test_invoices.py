...
C20  10× server errors → circuit opens (badge shown — frontend test)
...
from surge.api.invoices import _submit_invoice, _check_discount_limits, CreateInvoiceRequest, InvoiceItem, PaymentItem
from surge.api.auth import _hash_pin, request_approval, verify_approval_token, _sign_token
    ...
    if not frappe.db.exists("POS Profile", _PROFILE):
        ...
        for u, lvl, pin in [
            ...
            p.append("applicable_for_users", {
                ...
                "access_level": lvl, "surge_pos_pin": pin,
            })
...
def _make_req(items=None, payments=None, req_id=None, token=None):
    profile_modes = frappe.get_all(
        "POS Profile Payments", {"parent": _PROFILE}, pluck="mode_of_payment"
    )
...
class TestDiscountApproval(InvoiceTestBase):
    ...
    def test_C16_reuse_approval_token_rejected(self):
        ...
        result = frappe.parse_json(
            request_approval(
                pos_profile=_PROFILE, approver=_MANAGER,
                pin=_MANAGER_PIN_HASH, action="discount_override",
            ).data
        )
        token = result["token"]
    ...
    def test_C17_redis_down_token_rejected(self):
        ...
        payload = {
            "action": "discount_override", "approver": _MANAGER,
            "access_level": "Manager", "profile": _PROFILE,
            "ts": datetime.now().isoformat(), "meta": "",
        }
    ...
    def test_C18_meta_capped_at_500_chars(self):
        ...
        result = frappe.parse_json(
            request_approval(
                pos_profile=_PROFILE, approver=_MANAGER,
                pin=_MANAGER_PIN_HASH, action="discount_override", meta=big_meta,
            ).data
        )
        if result["status"] == "ok":
            token = result["token"]
