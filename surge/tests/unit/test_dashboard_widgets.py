...
def _recent_row(
    item_code="BEER-001", item_name="Beer", item_group="Spirits",
    rate=250.0, qty=2.0, invoice="SINV-001",
    posting_date="2026-01-15", status="Paid",
):
    return SimpleNamespace(
        item_code=item_code, item_name=item_name, item_group=item_group,
        rate=rate, qty=qty, invoice=invoice,
        posting_date=posting_date, status=status,
    )


def _low_row(
    item_code="BEER-001", item_name="Beer", warehouse="Stores",
    actual_qty=2.0, reorder_level=5.0, reorder_qty=20.0,
):
    return SimpleNamespace(
        item_code=item_code, item_name=item_name, warehouse=warehouse,
        actual_qty=actual_qty, reorder_level=reorder_level, reorder_qty=reorder_qty,
    )
...
class TestRecentItems(unittest.TestCase):
    ...
    def test_R05_response_shape(self):
        ...
        self.assertEqual(len(data), 1)
        row = data[0]
        for field in ("item_code", "item_name", "item_group", "rate", "qty", "invoice", "posting_date", "status"):
            self.assertIn(field, row, f"Missing field: {field}")

    def test_R06_multi_invoice_five_items_returned(self):
        """R06: 2 items from SINV-001 + 3 items from SINV-002 → all 5 returned."""
        rows = [
            _recent_row(item_code=f"ITEM-{i}", invoice="SINV-001" if i < 2 else "SINV-002")
            for i in range(5)
        ]
        data = _call_recent_items(_period_frappe(rows=rows))
...
class TestWidgetsData(unittest.TestCase):
    ...
    def test_W05_cache_hit_skips_all_sql(self):
        ...
        m.cache.return_value.get_value.return_value = {
            "top_products": [], "low_stock": [], "recent_items": [],
        }
        data = _call_widgets(m)
        m.db.sql.assert_not_called()
