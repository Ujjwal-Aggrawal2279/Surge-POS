...
sys.modules.setdefault("frappe.utils.password", MagicMock(
    get_encryption_key=lambda: "surge-test-hmac-secret-key-32chars!!"
))

# Import the pure HMAC helpers directly
from surge.api.auth import _sign_token, verify_approval_token, _get_hmac_secret, APPROVAL_TOKEN_TTL


# ── Helpers ───────────────────────────────────────────────────────────────────

def _make_payload(action="discount_override", approver="manager@test.com",
                  access_level="Manager", profile="TestProfile",
                  ts=None, meta="") -> dict:
    return {
        "action": action,
        "approver": approver,
