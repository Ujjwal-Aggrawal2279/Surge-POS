...
def flush_write_queue() -> None:
    ...
    for entry in pending:
        ...
        try:
            ...
            _safe_publish("surge:invoice_submitted", {
...
def resolve_conflict(conflict_name: str, resolution: str) -> dict:
    ...
    if resolution == "Approved — Force Submit":
        ...
        original_allow_negative = frappe.db.get_single_value(
        ...
        try:
            ...
            if not original_allow_negative:
                frappe.db.set_single_value(
                    "Stock Settings", "allow_negative_stock", original_allow_negative
                )
    ...
    for row in rows:
        try:
            ...
            for item in payload.get("items", []):
                if (
                    item.get("item_code") == item_code
                    and (item.get("warehouse") or "") == warehouse
                ):
...
def _create_conflict_record(entry: dict, error: ConflictError) -> None:
    ...
    _safe_publish("surge:conflict_created", {
